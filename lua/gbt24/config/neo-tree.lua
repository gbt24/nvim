...
local icons = require("gbt24.config.custom")
vim.fn.sign_define("DiagnosticSignError", { text = icons.icons.diagnostic.error, texthl = "DiagnosticSignError" })
vim.fn.sign_define("DiagnosticSignWarn", { text = icons.icons.diagnostic.warn, texthl = "DiagnosticSignWarn" })
vim.fn.sign_define("DiagnosticSignInfo", { text = icons.icons.diagnostic.info, texthl = "DiagnosticSignInfo" })
vim.fn.sign_define("DiagnosticSignHint", { text = icons.icons.diagnostic.hint, texthl = "DiagnosticSignHint" })
		...
		open_externally = function(state)
			...
			elseif node.ext == "jpg" or node.ext == "png" or node.ext == "pdf" or node.ext == "svg" then
				if vim.fn.has("mac") == 1 then
					...
					vim.fn.jobstart(
						"export DISPLAY=$(tmux show-env | sed -n 's/^DISPLAY=//p'); xdg-open "
							.. node.path
							.. " || evince "
							.. node.path
					)
